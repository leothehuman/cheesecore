...
translate([frame_size().x / 2 - extrusion_width(), 0, frame_size().z / 2]){
  mirror_y() {
    //translate([0, motor_pulley_link() + extrusion_width()/2, 0])
      //cube([1,1,1]);
      translate([panel_thickness() + extrusion_width() + NEMA_width(NEMAtypeXY())/2, motor_pulley_link() + extrusion_width()/2 , 0])
      aluminium_motor_mount();
      //steel_2020_motor_mount();
    translate([panel_thickness() + extrusion_width() + NEMA_width(NEMAtypeXY())/2, motor_pulley_link() , 0])  //FIXME what is 3.5?
