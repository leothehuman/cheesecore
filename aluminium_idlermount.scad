  ...
  translate([-extrusion_width($extrusion_type), -extrusion_width($extrusion_type), 0])
    color(alum_part_color()) {
      difference ()
      {
        ...
        translate ([extrusion_width($extrusion_type)/2,extrusion_width($extrusion_type)+move_holes_by, idler_height])
          linear_repeat(extent = [0, extrusion_screwholes_line_length, 0], count = 3) {
            clearance_hole(nominal_d = screwhole_M, h=50);
          }
      }
    }
}
